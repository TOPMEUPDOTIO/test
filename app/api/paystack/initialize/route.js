...
export async function POST(request) {
  ...
  try {
    ...
    const response = await fetch('https://api.paystack.co/transaction/initialize', { method: 'POST', headers: { Authorization: `Bearer ${process.env.PAYSTACK_SECRET_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ amount: Math.round(amount * 100), email: body.email || record.email || 'payments@topmeup.io', reference, callback_url: process.env.PAYSTACK_CALLBACK_URL || 'https://topmeup.io', metadata: { code: record.code, requestId: record.id } }) });
