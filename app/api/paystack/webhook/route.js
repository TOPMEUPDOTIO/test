...
export async function POST(request) {
  ...
  if (!process.env.PAYSTACK_SECRET_KEY || !signature || !crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(crypto.createHmac('sha512', process.env.PAYSTACK_SECRET_KEY).update(raw).digest('hex')))) return new Response('Unauthorized', { status: 401 });
  try {
